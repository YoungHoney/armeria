...
import com.linecorp.armeria.internal.server.annotation.DefaultDescriptiveTypeInfoProvider;
...
final class Toy {
    ...
    @JsonCreator
    Toy(@JsonProperty("toyName") String toyName,
...
final class Dog extends Mammal {
    ...
    @JsonCreator
    Dog(@JsonProperty("name") String name,
...
final class Cat extends Mammal {
    ...
    @JsonCreator
    Cat(@JsonProperty("name") String name,
        @JsonProperty("likesTuna") boolean likesTuna,
...
class JsonSchemaGeneratorTest {
    ...
    void testMethodWithRecursivePath() {
        ...
        final List<FieldInfo> parameters = ImmutableList.of(
                FieldInfo.of("param1", TypeSignature.ofBase("int")),
                FieldInfo.of("paramRecursive", TypeSignature.ofStruct("rec", commonTypeObjectForRecursion))
        );
        final StructInfo structInfo = new StructInfo(methodName, parameters);
        ...
        assertThatJson(methodSchema).node("definitions.rec.properties.inner-recurse.$ref")
                .isEqualTo("#/definitions/rec");
    }
    ...
    void shouldGenerateOneOfForPolymorphicType() {
        ...
        final DescriptiveTypeInfoProvider providerChain =
                new JacksonPolymorphismTypeInfoProvider()
                        .orElse(DocServiceTestUtil.newDefaultDescriptiveTypeInfoProvider(false));
        ...
        final MethodInfo testMethod = new MethodInfo(
                "animal-service", "animalMethod", 0, TypeSignature.ofBase("void"),
                ImmutableList.of(FieldInfo.of("animal", TypeSignature.ofStruct(Animal.class))),
        ...
        final ServiceSpecification specification = new ServiceSpecification(
                ...
                ImmutableList.of(),
                ImmutableList.of(animalInfo, dogInfo, catInfo, toyInfo, mammalInfo),
                ImmutableList.of());
    ...
    private static ServiceSpecification newServiceSpecificationWithRequestStruct(StructInfo... structInfos) {
        // [수정] MethodInfo의 유효한 생성자를 사용하도록 변경
        final MethodInfo methodInfo = new MethodInfo(
                "test-service",
                methodName,
                0, // overloadId
                TypeSignature.ofBase("void"), // returnType
                ImmutableList.of(FieldInfo.of("request", TypeSignature.ofStruct(methodName, new Object()))),
